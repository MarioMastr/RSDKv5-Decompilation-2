...
void SKU::SteamUserStorage::CopySave(bool save, const char *filename) {
    ...
    if (SteamUser()->GetUserDataFolder(steamFolderPath, sizeof(steamFolderPath))) {
        std::string file(steamFolderPath);
        size_t pos = file.find("/local"); // linux stores in .local so we need the /
        std::string steamFilePath = file.erase(pos).append("/remote/").append(filename);
